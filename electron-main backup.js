...
async function saveAgentConfig(config) {
  ...
  const path = await import('path').then(module => module);
  ...
  console.log('💾 Saving PERSISTENT config to .env:');
  console.log('   Agent ID:', agentId);
  console.log('   Agent Token:', agentToken ? 'YES' : 'NO');
  console.log('   API Key:', apiKey ? 'YES' : 'NO');

  // **.env HARUS SIMPAN SEMUA YANG DIBUTUHKAN AGENT!**
  const envContent = `# Printer Agent Configuration - PERSISTENT
# DO NOT DELETE - Agent needs this to reconnect after restart
...
AGENT_LOCATION=${config.location || 'Unknown'}

# ===== SYSTEM =====
NODE_ENV=production
`;

  // Simpan ke .env di CURRENT DIRECTORY (untuk agent baca)
  const currentDirEnv = join(__dirname, '.env');
  fs.writeFileSync(currentDirEnv, envContent, 'utf8');
  console.log('✅ .env saved to:', currentDirEnv);

  // Juga simpan ke userData untuk backup
  const userDataPath = app.getPath('userData');
  const userDataEnv = join(userDataPath, '.env');
  fs.writeFileSync(userDataEnv, envContent, 'utf8');
  console.log('✅ Backup .env saved to:', userDataEnv);
...
function startAgent() {
  ...
  console.log('🔍 CURRENT CONFIG KEYS:');
  console.log('   agentId:', config.agentId);
  console.log('   apiKey:', config.apiKey ? 'YES' : 'NO');
  console.log('   agentToken:', config.agentToken ? 'YES' : 'NO');
  console.log('   backendUrl:', config.backendUrl);

  // **JANGAN CREATE .env DI SINI! SUDAH ADA saveAgentConfig() UNTUK ITU!**
  // **CEK SAJA .env SUDAH ADA ATAU BELUM**
  const envPath = join(__dirname, '.env');

  if (!existsSync(envPath)) {
    console.error('❌ .env file not found! Agent cannot start.');
    console.log('   Please register agent first or restart Electron');
    return;
  }

  // **BACA .env YANG SUDAH ADA**
  const envContent = readFileSync(envPath, 'utf8');
  const agentTokenMatch = envContent.match(/AGENT_TOKEN=([^\n]+)/);
  const apiKeyMatch = envContent.match(/CLOUD_API_KEY=([^\n]+)/);
  const wsUrlMatch = envContent.match(/CLOUD_WS_URL=([^\n]+)/);

  const agentToken = agentTokenMatch ? agentTokenMatch[1].trim() : config.agentToken || '';
  const apiKey = apiKeyMatch ? apiKeyMatch[1].trim() : config.apiKey || '';
  let websocketUrl = wsUrlMatch ? wsUrlMatch[1].trim() : (config.websocketUrl || 'ws://localhost:3001/ws/agent');

  // Fix WebSocket URL jika masih port 5000
  if (websocketUrl.includes(':5000')) {
    websocketUrl = websocketUrl.replace(':5000', ':3001');
  }

  console.log('🚀 Starting agent with config from .env:');
  console.log('   AGENT_TOKEN:', agentToken ? `${agentToken.substring(0, 10)}...` : 'MISSING!');
  console.log('   CLOUD_WS_URL:', websocketUrl);
  console.log('   AGENT_ID:', config.agentId);

  // **JANGAN WRITE .env LAGI! SUDAH ADA!**

  // Start agent process dengan env dari .env
