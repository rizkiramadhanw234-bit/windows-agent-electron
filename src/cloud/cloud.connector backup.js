...
export class CloudConnector {
  ...
  connect() {
    if (!this.apiKey) {
      console.error("❌ Cloud API key not configured");
      return;
    }

    console.log(`🔗 Connecting to backend: ${this.cloudUrl}`);
    ...
    this.ws = new WebSocket(this.cloudUrl, { headers });
    ...
    this.ws.on("open", () => {
      ...
      // Kirim device_register dengan agentId
      this.send({
